...
_Обновлено: 2026-08-23. Для агента-преемника._
...
**Статус: в проде, проверено доставкой 2026-08-23.** Плановый запуск ежедневно 09:00 МСК (cron `0 6 * * *` UTC).
...
| `config/config.yaml` | Источники: 3 платформы + 14 RSS-лент; режим отчёта `current`; таймзона Europe/Moscow; расписание (schedule) выключено |
